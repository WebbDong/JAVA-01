...
>>   - 不会立即无用，但是也不会使用太长时间
>>
>> 根据这个情况就形成了弱分代假设理论，这一假设任为”大多数对象在年轻时死亡“。
...
> ### 3.4.1 新生代 (Eden Space)
>> 新生代也叫伊甸区，是堆内存中的一个区域，用来分配新创建的对象。通常情况会有多个线程并发的创建多个对象，出现线程安全问题，
>> 这样就会产生指针碰撞问题。
>>
>> 指针碰撞: 假设 JVM 上堆内存现在是规整的，没有碎片。堆内存被指针一分为二，左边是已经使用的内存区域，右边是未使用的内存区域，
...
>>
>> 对象间可能存在跨代引用，比如老年代对象引用年轻代对象。HotSpot JVM 使用卡片标记 (Card Marking) 来解决这个问题。避免每次 Young GC 时
>> 扫描整个老年代。
>> 
>> 卡片标记: 具体是使用卡表 (Card Table) 和写屏障 (Write Barrier) 来进行标记并加快对 GC Root 的扫描。
>
>
>
>
